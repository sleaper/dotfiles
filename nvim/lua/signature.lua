...
source.get_trigger_characters = function(self)
	...
	for _, c in
		ipairs(
			self:_get(self:_get_client().server_capabilities, { "signatureHelpProvider", "triggerCharacters" }) or {}
		)
	do
	...
	for _, c in
		ipairs(
			self:_get(self:_get_client().server_capabilities, { "signatureHelpProvider", "retriggerCharacters" }) or {}
		)
	do
		table.insert(trigger_characters, c)
	end
...
source._signature_label = function(self, signature, parameter_index)
	...
	if parameter_index then
		local s, e =
			string.find(label, self:_parameter_label(signature, signature.parameters[parameter_index]), 1, true)
		if s and e then
